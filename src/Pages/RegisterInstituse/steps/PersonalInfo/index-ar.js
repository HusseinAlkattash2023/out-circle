...
const PartnerInfoAr = ({ num , setNum , setData , data  , formErrors})=>{
    ...
    return(
        <div>
                ...
                <div className='my-3 input_'>
                    <input value={data.password} onChange={(e) => setData({...data , password:e.target.value})}   placeholder="كلمة المرور" type="password"/><br/>
                    {formErrors.password && (<span className="error" style={{ color: "red" }}>{formErrors.password}</span>)}
                </div>
                <div className='my-3 input_'>
                    <input value={data.confirm_password} onChange={(e) => setData({...data , confirm_password:e.target.value})}   placeholder="تأكيد كلمة المرور" type="password"/><br/>
                    {formErrors.confirm_password && (<span className="error" style={{ color: "red" }}>{formErrors.confirm_password}</span>)}
                </div>
                <div className='my-3 input_'>
