import React from 'react'
import MessageAr from '../../../Components/Message/Message-Ar';
import image from '../../../Assets/images/personal_service-ar/image7.png';
const PersonalServiceAr_7 = () => {
  return (
    <div>
        <MessageAr 
        gmail={"p.individuals.outcircle@gmail.com"}
        admin={"outcircle2023@gmail.com"}
        change_route={"personal-service/service7-ar"}
        image={image} text={"خدمات الطاقة البديلة"}
        head={"أفراد"} address={"خدمات الطاقة البديلة"}
        text1={"تقديم النصائح الصادقة حول المنتجات المناسبة للطاقة البديلة وتقييم الاحتياجات الحقيقية لكل شخص"}
        text2={"المساعدة في تأمين منظومات طاقة بديلة بأسعار منطقية ومنافسة ومضمونة"}
        text3={"المساعدة في تحديث المنظومات الحالية أو استبدالها بتكلفة منطقية ومناسبة"}
        text4={"...."}
        text5={"...."}
        text6={"...."}
        text70={"...."}
        />
    </div>
  )
}

export default PersonalServiceAr_7;